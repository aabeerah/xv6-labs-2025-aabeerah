...
// ------------------------------------------------------------
// Return the time quantum associated with a given MLFQ level.
// ------------------------------------------------------------
static int
mlfq_get_quantum(int level)
{
  switch(level) {
    case 0: return L0;
    case 1: return L1;
...
// ------------------------------------------------------------
// Insert a process at the tail of a given MLFQ level.
...
mlfq_add_to_queue(struct proc *p, int new_level)
{
  ...
  // FIFO insert at tail
  if(mlfq[new_level].tail == 0) {
    // Queue empty
    mlfq[new_level].head = p;
    mlfq[new_level].tail = p;
  } else {
    // Append to tail
    mlfq[new_level].tail->next_proc = p;
    mlfq[new_level].tail = p;
  }
}

// ------------------------------------------------------------
// Remove a process from its current MLFQ queue.
// Caller must hold p->lock.
// ------------------------------------------------------------
static void
mlfq_detach_from_queue(struct proc *p)
{
  ...
  struct proc *iter = mlfq[level].head;
  struct proc *prev = 0;
  ...
  while(iter) {
    if(iter == p) {
      ...
      // Removing from head
      if(prev == 0) {
        mlfq[level].head = iter->next_proc;
      } else {
        prev->next_proc = iter->next_proc;
      }
...
mlfq_select_next(void)
{
  ...
  for(int q = 0; q < MLFQ_LEVELS; q++) {
    ...
    struct proc *candidate = mlfq[q].head;
    ...
    while(candidate) {
      ...
      if(candidate->state == RUNNABLE) {
        ...
        // Remove from queue before giving to CPU
        mlfq_detach_from_queue(candidate);

        return candidate;  // return with lock held
      }

      release(&candidate->lock);
      candidate = candidate->next_proc;
    }
  }
...
mlfq_demote_process(struct proc *p)
{
  ...
  p->quantum    = mlfq_get_quantum(p->queue_level);
  p->ticks_used = 0;
}

// ------------------------------------------------------------
// Promote (boost) every process back to top queue.
// Used to prevent starvation.
// ------------------------------------------------------------
void
mlfq_global_boost(void)
{
  struct proc *p;

  // Iterate over entire process table
  for(p = proc; p < &proc[NPROC]; p++) {
    acquire(&p->lock);

    if(p->state == RUNNABLE || p->state == RUNNING) {

      // Detach from current queue if applicable
      if(p->queue_level >= 0 && p->queue_level < MLFQ_LEVELS)
        mlfq_detach_from_queue(p);
